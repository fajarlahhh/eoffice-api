    ...
    try {
        getPelanggan = (uid) => {
            return new Promise((resolve, reject) => {
                db.query('SELECT * FROM (pengguna a LEFT JOIN (personalia.pegawai b LEFT JOIN personalia.bagian c ON b.kd_bagian=c.kd_bagian) ON a.nip=b.nip) LEFT JOIN level_pengguna d ON a.id=d.id_pengguna WHERE a.nip = ?', uid, (error, elements) => {
                    if (error) {
                        return reject(error);
                    }
        ...
        const result = await getPelanggan(uid)
